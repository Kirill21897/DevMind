# DevMind AI Агент

DevMind — это продвинутый локальный ИИ-ассистент, разработанный специально для программистов и инди-разработчиков. Он использует локальные LLM (через Ollama), технологию RAG (Retrieval-Augmented Generation) и автономные агентные возможности для помощи в написании кода, работе с документацией и поиске информации.

## Возможности

-   **Локальный Интеллект**: Работает на базе Ollama (поддерживает модели `qwen2.5-coder`, `llama3` и другие).
-   **Умный RAG Пайплайн**:
    -   Двухэтапный поиск: Векторный поиск (ChromaDB) + Переранжирование (Cross-Encoder).
    -   Динамический выбор модели эмбеддингов (например, `nomic-embed-text`, `mxbai-embed-large`).
    -   Индексация локальной документации в формате Markdown.
-   **Автономные Инструменты**:
    -   `create_plan`: Создание пошагового плана действий для сложных задач.
    -   `retrieve_knowledge`: Поиск по локальной базе знаний.
    -   `web_search`: Поиск в интернете (через DuckDuckGo).
    -   `save_solution`: Сохранение сгенерированного кода и гайдов в файлы.

## Архитектура Агента (ReAct)

DevMind построен на архитектуре **ReAct (Reasoning + Acting)**, что позволяет ему не просто отвечать на вопросы, а действовать как автономный инженер:

1.  **Reasoning (Рассуждение)**: Перед каждым действием агент анализирует текущую задачу и решает, какой шаг предпринять следующим.
2.  **Planning (Планирование)**: Для сложных задач агент использует инструмент `create_plan`, чтобы разбить работу на этапы.
3.  **Acting (Действие)**: Агент вызывает подходящие инструменты (поиск, чтение файлов, сохранение кода).
4.  **Reflection (Рефлексия)**: После выполнения действия агент оценивает результат и корректирует план при необходимости.

Этот цикл продолжается до полного решения задачи (максимум 10 шагов), обеспечивая глубокую проработку запросов.
-   **Современный UI (Streamlit)**:
    -   **Управление базой знаний**: Загрузка файлов и переиндексация одной кнопкой прямо из интерфейса.
    -   **Гибкие настройки**: Смена LLM и модели эмбеддингов "на лету", редактирование системного промпта.
    -   **История чата**: Сохранение контекста и удобная очистка.
-   **Наблюдаемость (Observability)**: Полный трекинг и отладка через [LangFuse](https://langfuse.com/).
-   **Оценка Качества**: Встроенный пайплайн оценки Ragas для проверки точности контекста и релевантности ответов.

## Структура Проекта

```
├── app.py                  # Точка входа веб-приложения (Streamlit)
├── main.py                 # Точка входа CLI (командная строка)
├── src/
│   ├── agent.py            # Логика агента (ReAct цикл)
│   ├── config.py           # Настройки конфигурации
│   ├── tools.py            # Инструменты агента
│   └── utils.py            # Утилиты
├── scripts/
│   ├── ingest_data.py      # Скрипт индексации документов в ChromaDB
│   └── evaluate_rag.py     # Скрипт оценки качества (Ragas)
├── docs/
│   └── completed/          # История разработки и документация
└── requirements.txt        # Зависимости Python
```

## Установка и Настройка

### Требования

1.  **Python 3.10+**
2.  **Ollama**: Установите и запустите [Ollama](https://ollama.com/).
    -   Скачайте необходимые модели:
        ```bash
        ollama pull qwen2.5-coder:14b  # Или ваша предпочтительная модель
        ollama pull nomic-embed-text   # Для эмбеддингов
        ```

### Установка

1.  Клонируйте репозиторий.
2.  Создайте виртуальное окружение:
    ```bash
    python -m venv .venv
    # Windows
    .venv\Scripts\activate
    # Linux/Mac
    source .venv/bin/activate
    ```
3.  Установите зависимости:
    ```bash
    pip install -r requirements.txt
    ```

### Конфигурация

Создайте файл `.env` (необязательно), чтобы переопределить настройки по умолчанию:

```env
# Настройки LLM
LLM_MODEL=qwen2.5-coder:14b
EMBEDDING_MODEL=nomic-embed-text
OLLAMA_BASE_URL=http://localhost:11434/v1

# LangFuse (Опционально)
LANGFUSE_PUBLIC_KEY=pk-lf-...
LANGFUSE_SECRET_KEY=sk-lf-...
LANGFUSE_HOST=https://cloud.langfuse.com
```

## Использование

### 1. Веб-интерфейс (Рекомендуется)

Запустите Streamlit UI:

```bash
streamlit run app.py
```

Чат будет доступен по адресу `http://localhost:8501`.

**Функции интерфейса:**
*   **Чат**: Общение с агентом. Агент автоматически определяет язык (Русский/Английский).
*   **Knowledge Base (Сайдбар)**:
    *   Загружайте новые документы (`.md`, `.txt`, `.pdf`).
    *   Нажмите `Re-index Knowledge Base`, чтобы обновить базу знаний с текущей моделью эмбеддингов.
*   **Settings (Сайдбар)**:
    *   `Ollama Model`: Введите название модели (например, `llama3`), и агент мгновенно переключится.
    *   `Embedding Model`: Измените модель векторизации для поиска.
    *   `System Prompt`: Редактируйте инструкции агента в реальном времени.

### 2. Командная строка (CLI)

Выполнить одиночный запрос:

```bash
python main.py --query "Как реализовать RAG пайплайн?"
```

Запустить интерактивную сессию:

```bash
python main.py
```

### 3. Оценка Качества (Evaluation)

Для запуска оценки Ragas на основе собранных логов:

```bash
python scripts/evaluate_rag.py
```

## Лицензия

MIT
